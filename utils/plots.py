...
from data import distance_bins, azimuth_bins, elevation_bins, get_names_learning, get_num_learning_labels
...
def draw_distance_and_label_matrix(matrix: np.array, save_dir: str=None):
    ...
    fig, ax = plt.subplots()
    im = ax.pcolor(matrix, norm=colors.LogNorm(vmin=1, vmax=np.max(matrix)), cmap=plt.cm.Blues)
...
def draw_azimuth_and_label_matrix(matrix: np.array, save_dir: str=None):
    ...
    ax.set_xticklabels(np.append(azimuth_bins, 360), ha='right', rotation=45, rotation_mode='anchor')
    ax.xaxis.set_label_position('bottom')
    ax.xaxis.tick_bottom()
...
def draw_elevation_and_label_matrix(matrix: np.array, save_dir: str=None):
    ...
    ax.set_xticks(np.arange(matrix.shape[1]))
    ax.set_xticklabels(elevation_bins, ha='right', rotation=45,
...
def draw_points_per_label(ppl: np.array, save_dir: str=None):
    ...
    ax.set_xticklabels(get_names_learning()[0], ha='right', rotation=45, rotation_mode='anchor')
...
def draw_distance_and_sequence_matrix(matrix: np.array, save_dir: str=None):
    ...
    fig, ax = plt.subplots()
    im = ax.pcolor(matrix.T, norm=colors.LogNorm(vmin=1, vmax=np.max(matrix)), cmap=plt.cm.Blues)
    cbar = ax.figure.colorbar(im, ax=ax)
    cbar.ax.set_ylabel('Number of Points', rotation=90)

    ax.set_xlabel('Sequence')
    ax.set_xticks(np.arange(matrix.shape[0]))
    ax.set_xticklabels(['{:02d}'.format(i) for i in range(11)], ha='center')
...
def draw_sequence_length(save_dir: str=None):
    length = [4541, 1101, 4661, 801, 271, 2661, 1101, 1101, 4071, 1591, 1201,
    ...
    ax.set_xticklabels(['{:02d}'.format(i) for i in range(22)], ha='center')
